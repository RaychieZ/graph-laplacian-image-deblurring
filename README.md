...
Let `x` be the unknown clean image, `K` be a Gaussian blur operator, and `b_delta` be the blurred/noisy observation:

```math
b^\delta = Kx_{true} + \eta.
```

The restoration is computed by solving

```math
\min_x \; \|Kx - b^\delta\|_2^2 + \mu x^T L x,
```

where `L` is a weighted graph Laplacian built from the observed blurred/noisy image. The normal equation is

```math
(K^T K + \mu L)x = K^T b^\delta.
```
